...
interface AppState {
  ...
  autoRetryConcurrency: number
  setAutoRetryConcurrency: (concurrency: number) => void
  autoRetrySession: AutoRetrySession | null
  stopAutoRetrySession: () => void
  autoRetrySummaryModal: AutoRetrySummaryModalData | null
  closeAutoRetrySummaryModal: () => void
...
export const useStore = create<AppState>()(
  persist(
    (set, get) => ({
      ...
      setTargetSuccessCount: (count) => set({ targetSuccessCount: Math.max(0, count) }),
      autoRetryConcurrency: 1,
      setAutoRetryConcurrency: (concurrency) => set({ autoRetryConcurrency: Math.max(1, concurrency) }),
      autoRetrySession: null,
      stopAutoRetrySession: () => {
        const session = get().autoRetrySession
        if (session && session.active) {
          const durationSeconds = Math.max(1, Math.round((Date.now() - session.startTime) / 1000))
          const summaryData: AutoRetrySummaryModalData = {
            targetCount: session.targetSuccessCount,
            successCount: session.currentSuccessCount,
          ...
          set({
            autoRetrySession: { ...session, active: false, draining: false },
            autoRetrySummaryModal: summaryData,
          })
          get().showToast('已停止挂机连续生图并生成统计报告', 'info')
        }
      },
      autoRetrySummaryModal: null,
      closeAutoRetrySummaryModal: () => set({ autoRetrySummaryModal: null }),
...
export async function submitTask(options: { allowFullMask?: boolean; useCurrentApiProfileWhenReusedMissing?: boolean } = {}) {
  ...
  const targetSuccessCount = useStore.getState().targetSuccessCount
  const autoRetryConcurrency = Math.max(1, useStore.getState().autoRetryConcurrency || 1)

  if (targetSuccessCount > 0) {
    const existingSession = useStore.getState().autoRetrySession
    if (existingSession?.active) {
      showToast('已有挂机连续生图任务在运行中，请先停止当前挂机', 'info')
      return
    }

    const sessionId = genId()
    const initialSession: AutoRetrySession = {
      sessionId,
      targetSuccessCount,
      concurrency: autoRetryConcurrency,
    ...
    useStore.setState({ autoRetrySession: initialSession })
    showToast(`启动挂机连续生图：目标 ${targetSuccessCount} 张，并发 ${autoRetryConcurrency}`, 'success')
...
async function executeTask(taskId: string) {
  ...
  try {
    ...
    if (latestTask.apiProvider === 'fal' && latestFalRequestInfo && isNetworkRecoverableError(err)) {
      ...
      if (!latestTask.autoRetrySessionId) {
...
function handleAutoRetryTaskCompletion(taskId: string, status: TaskRecord['status'], outputCount: number) {
  if (status === 'running') return
  const state = useStore.getState()
  const session = state.autoRetrySession
  if (!session || !session.active) return
  const task = state.tasks.find((t) => t.id === taskId)
  if (!task || task.autoRetrySessionId !== session.sessionId) return
  if (!session.activeTaskIds.includes(taskId)) return

  const isSuccess = status === 'done' && outputCount > 0
  const updatedSuccessCount = session.currentSuccessCount + (isSuccess ? outputCount : 0)
  const updatedFailedAttempts = session.failedAttempts + (isSuccess ? 0 : 1)
  const remainingActiveTaskIds = session.activeTaskIds.filter((id) => id !== taskId)
  const isTargetMet = updatedSuccessCount >= session.targetSuccessCount
  const isDraining = session.draining || isTargetMet

  // 挂机模式下，若某次生图请求失败（或未生成图片），自动清理该失败任务卡片，避免列表中堆积废图；成功生图则保留
  if (!isSuccess) {
    void removeTasks([taskId])
  }

  // 若达成目标，进入/保持收尾状态，不再发起新请求，等待其它已在跑的并发任务全部完成结算
  if (isDraining) {
    if (remainingActiveTaskIds.length > 0) {
      useStore.setState({
        autoRetrySession: {
          ...session,
          currentSuccessCount: updatedSuccessCount,
          failedAttempts: updatedFailedAttempts,
          activeTaskIds: remainingActiveTaskIds,
          draining: true,
        },
      })
      return
    ...
    // 剩余在跑任务全部收尾完毕，正式结束挂机并弹窗总结 Modal
    const durationSeconds = Math.max(1, Math.round((Date.now() - session.startTime) / 1000))
    const summaryData: AutoRetrySummaryModalData = {
      targetCount: session.targetSuccessCount,
    ...
    const msg = `挂机连续生图完成！已成功生成 ${updatedSuccessCount} 张图片，所有并发请求已全部收尾。`
    useStore.getState().showToast(msg, 'success')
    showTaskCompletionNotification('挂机连续生图已完成', msg)
    return
  }
...
async function dispatchAutoRetryTasks() {
  const state = useStore.getState()
  const session = state.autoRetrySession
  if (!session || !session.active || session.draining) return
  if (session.currentSuccessCount >= session.targetSuccessCount) return

  const activeRunningCount = session.activeTaskIds.filter((id) => {
    const t = state.tasks.find((task) => task.id === id)
    return t && t.status === 'running'
  }).length

  // 只要未达成目标成功数且未在收尾阶段，补满并发槽位
  const needed = session.concurrency - activeRunningCount
  ...
  for (let i = 0; i < needed; i++) {
    const currentSession = useStore.getState().autoRetrySession
    if (!currentSession || !currentSession.active) break
    if (currentSession.currentSuccessCount >= currentSession.targetSuccessCount) break

    const taskId = genId()
    const now = Date.now()
    const task: TaskRecord = {
      id: taskId,
      autoRetrySessionId: currentSession.sessionId,
      ...
      apiMode: activeProfile.apiMode,
      apiModel: activeProfile.model,
      inputImageIds: currentSession.inputImageIds,
      maskTargetImageId: currentSession.maskTargetImageId,
      maskImageId: currentSession.maskImageId,
      outputImages: [],
      status: 'running',
      error: null,
      createdAt: now,
    ...
    useStore.setState((st) => {
      const cur = st.autoRetrySession
      if (!cur || cur.sessionId !== currentSession.sessionId) return st
      return {
        tasks: [task, ...st.tasks],
        autoRetrySession: {
          ...cur,
          totalAttempts: cur.totalAttempts + 1,
          activeTaskIds: [...cur.activeTaskIds, taskId],
        },
      }
    })
...
export function updateTaskInStore(taskId: string, patch: Partial<TaskRecord>) {
  ...
  if (task) {
    ...
    if (task.status === 'done' || task.status === 'error') {
      handleAutoRetryTaskCompletion(task.id, task.status, task.outputImages.length)
    }
  }
}
