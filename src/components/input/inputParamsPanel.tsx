...
export default function InputParamsPanel({
  ...
  qualityHint,
  onOpenSizePicker,
  targetSuccessCountInput,
  setTargetSuccessCountInput,
  commitTargetSuccessCount,
  autoRetryConcurrencyInput,
  setAutoRetryConcurrencyInput,
  commitAutoRetryConcurrency,
}: {
  cols: string
  params: TaskParams
  ...
  targetSuccessCountInput: string
  setTargetSuccessCountInput: (value: string) => void
  commitTargetSuccessCount: () => void
  autoRetryConcurrencyInput: string
  setAutoRetryConcurrencyInput: (value: string) => void
  commitAutoRetryConcurrency: () => void
}) {
  return (
    <div className={`grid ${cols} gap-2 text-xs flex-1`}>
      ...
      <label className="relative flex flex-col gap-0.5">
        <span className="text-gray-400 dark:text-gray-500 ml-1">目标成功</span>
        <input
          value={targetSuccessCountInput}
          onChange={(e) => setTargetSuccessCountInput(e.target.value)}
          onBlur={commitTargetSuccessCount}
          type="number"
          min={0}
          max={100}
          placeholder="0(关闭)"
          ...
          title="设定需要成功生成的图片张数，>0时开启自动挂机重试"
        />
      </label>
      <label className="relative flex flex-col gap-0.5">
        <span className="text-gray-400 dark:text-gray-500 ml-1">并发数</span>
        <input
          value={autoRetryConcurrencyInput}
          onChange={(e) => setAutoRetryConcurrencyInput(e.target.value)}
          onBlur={commitAutoRetryConcurrency}
          type="number"
          min={1}
          max={10}
          placeholder="1"
          className="px-3 py-1.5 rounded-xl border border-gray-200/60 dark:border-white/[0.08] focus:outline-none text-xs transition-all duration-200 shadow-sm bg-white/50 dark:bg-white/[0.03]"
          title="挂机生图时的最大并发请求数量"
        />
      </label>
    </div>
  )
