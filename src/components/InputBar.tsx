...
export default function InputBar() {
  ...
  const targetSuccessCount = useStore((s) => s.targetSuccessCount)
  const setTargetSuccessCount = useStore((s) => s.setTargetSuccessCount)
  const autoRetryConcurrency = useStore((s) => s.autoRetryConcurrency)
  const setAutoRetryConcurrency = useStore((s) => s.setAutoRetryConcurrency)
  const autoRetrySession = useStore((s) => s.autoRetrySession)
  const stopAutoRetrySession = useStore((s) => s.stopAutoRetrySession)

  const [targetSuccessCountInput, setTargetSuccessCountInput] = useState(targetSuccessCount ? String(targetSuccessCount) : '')
  useEffect(() => {
    setTargetSuccessCountInput(targetSuccessCount ? String(targetSuccessCount) : '')
  }, [targetSuccessCount])

  const commitTargetSuccessCount = useCallback(() => {
    const val = parseInt(targetSuccessCountInput.trim(), 10)
    if (isNaN(val) || val <= 0) {
      setTargetSuccessCount(0)
      setTargetSuccessCountInput('')
    } else {
      const clamped = Math.min(100, Math.max(1, val))
      setTargetSuccessCount(clamped)
      setTargetSuccessCountInput(String(clamped))
    }
  }, [targetSuccessCountInput, setTargetSuccessCount])

  const [autoRetryConcurrencyInput, setAutoRetryConcurrencyInput] = useState(String(autoRetryConcurrency || 1))
  useEffect(() => {
    setAutoRetryConcurrencyInput(String(autoRetryConcurrency || 1))
  }, [autoRetryConcurrency])

  const commitAutoRetryConcurrency = useCallback(() => {
    const val = parseInt(autoRetryConcurrencyInput.trim(), 10)
    if (isNaN(val) || val <= 1) {
      setAutoRetryConcurrency(1)
      setAutoRetryConcurrencyInput('1')
    } else {
      const clamped = Math.min(10, Math.max(1, val))
      setAutoRetryConcurrency(clamped)
      setAutoRetryConcurrencyInput(String(clamped))
    }
  }, [autoRetryConcurrencyInput, setAutoRetryConcurrency])
  ...
  const renderParams = (cols: string) => (
    <InputParamsPanel
      ...
      targetSuccessCountInput={targetSuccessCountInput}
      setTargetSuccessCountInput={setTargetSuccessCountInput}
      commitTargetSuccessCount={commitTargetSuccessCount}
      autoRetryConcurrencyInput={autoRetryConcurrencyInput}
      setAutoRetryConcurrencyInput={setAutoRetryConcurrencyInput}
      commitAutoRetryConcurrency={commitAutoRetryConcurrency}
    />
  )
  ...
  return (
    <>
      ...
      >
        {autoRetrySession?.active && (
          <div className="flex justify-center mb-3">
            <div className="bg-blue-600/90 dark:bg-blue-900/90 text-white backdrop-blur shadow-[0_8px_30px_rgb(0,0,0,0.12)] rounded-full flex items-center px-4 py-1.5 text-xs border border-blue-400/30 gap-3 pointer-events-auto">
              <span className="inline-flex items-center gap-1.5 font-medium">
                <span className="relative flex h-2 w-2">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-sky-400 opacity-75"></span>
                  <span className="relative inline-flex rounded-full h-2 w-2 bg-sky-300"></span>
                </span>
                {autoRetrySession.draining
                  ? `已达标 (${autoRetrySession.currentSuccessCount}/${autoRetrySession.targetSuccessCount}张)，正在等待剩余 ${autoRetrySession.activeTaskIds.length} 个并发请求收尾...`
                  : `挂机连续生图中：成功 ${autoRetrySession.currentSuccessCount} / ${autoRetrySession.targetSuccessCount} 张`}
              </span>
              <span className="opacity-80">| 并发: {autoRetrySession.concurrency}</span>
              <span className="opacity-80">| 请求: {autoRetrySession.totalAttempts} 次</span>
              <span className="opacity-80">| 失败: {autoRetrySession.failedAttempts} 次</span>
              <button
                type="button"
                onClick={() => stopAutoRetrySession()}
                className="ml-1 px-2.5 py-0.5 bg-red-500/80 hover:bg-red-600 rounded-full font-bold text-[11px] transition-colors"
              >
                停止挂机
              </button>
            </div>
